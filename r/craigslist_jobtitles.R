...
job.titles.path = "https://raw.githubusercontent.com/h2oai/sparkling-water/rel-1.6/examples/smalldata/craigslistJobTitles.csv?_sm_au_=iVVQjkNP1nvqsr7R"

jt <- read.csv(url(job.titles.path))
job.titles <- as.h2o(jt,
                     destination_frame = "jobtitles",
                     col.names = c("category", "jobtitle"), 
                     col.types = c("String", "String"), header = TRUE)
)

job.titles$jobtitle <- as.character(job.titles$jobtitle)
job.titles$category <- as.character(job.titles$category)
job.titles
